    ...
    <body>
        ...
        <hr>
            @for($i=0;$i<count($graficasGuardadas);$i++)
                <h1>{{$graficasGuardadas[$i]["titulo"]}}</h1>
                <p>Periodo: <b>{{$graficasGuardadas[$i]["periodo"]}}</b></p>
                <p>
                    Los cuestionario de esta grafica estan agrupados por: 
                    <b>Calificacion
                        @if($graficasGuardadas[$i]["agrupacion"]=="Cuestionario")Total
                        @elseif($graficasGuardadas[$i]["agrupacion"]=="Categori")por Categori ({{$graficasGuardadas[$i]["categoria"][$graficasGuardadas[$i]["subAgrupacion"]]['nombre']}})
                        @elseif($graficasGuardadas[$i]["agrupacion"]=="Dominio")por Dominio ({{$graficasGuardadas[$i]["dominio"][$graficasGuardadas[$i]["subAgrupacion"]]['nombre']}})
                        @endif
                    </b>
                </p>
                @if($graficasGuardadas[$i]['etiquetasLimitadores']!=null)
                <p>
                    La población esta limitada por los siguientes datos personales: 
                    <b>
                        @for($x=0;$x<count($graficasGuardadas[$i]['etiquetasLimitadores']);$x++)
                            @for($y=0;$y<count($graficasGuardadas[$i]['etiquetasLimitadores'][$x]['ids']);$y++)
                                {{$graficasGuardadas[$i]['etiquetasLimitadores'][$x]['ids'][$y]['name']}} 
                            @endfor
                        @endfor
                    </b>
                </p>
                @endif
                <div style="width: 40%">
                    <canvas id="myChart{{$i}}" width="50" height="25"></canvas>
                </div>
                <hr>
            @endfor
    </body>
...
<script type="text/javascript" >
    ...
    function PintarGraficas(){
        var graficasGuardadas = {!! json_encode($graficasGuardadas) !!};
